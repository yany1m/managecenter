	...
	function cashflowStatementToJson(){
		...
		$("#table").find("tr").each(function(){
			...
			if($.trim(arrtd.eq(0).text())!="" && $.trim(arrtd.eq(0).attr("role"))!="titleitem"){
				json1="{\"id\":"+arrtd.eq(0).attr("id")+",\"pid\":"+arrtd.eq(0).attr("pid")+",\"role\":"+"\""+arrtd.eq(0).attr("role")+"\""+",\"name\":"+"\""+$.trim(arrtd.eq(0).text())+"\""+",\"body\":"+"{\"本年累计金额\":"+"\""+arrtd.eq(1).find("input").val()+"\""+",\"上年累计金额\":"+"\""+arrtd.eq(2).find("input").val()+"\""+"}}";
				Jsontext+=json1+",";
			}else if($.trim(arrtd.eq(0).text())!=""){
		...
		Jsontext+="]";
		var dataset = $.parseJSON(Jsontext);
		var jsondata=createTree2(dataset,0,1);
		return jsondata;
	}
	...
	//生成map嵌套的json树
	 ...
	 //生成map嵌套的json树
	 function createTree2(jsons,pid,z){
		 var obj = {}, temp;
		 for(var i=0;i<jsons.length;i++){
			 if(jsons[i].pid == pid){
				 //var obj = {};      
				 if(jsons[z].role=='titleitem'){
					 obj['本年累计金额']=jsons[z].本年累计金额;
					 obj['上年累计金额']=jsons[z].上年累计金额;
				 }
				 temp =createTree1(jsons,jsons[i].id,i) ;
				 if (!isEmptyObject(temp)) {                   	
					 obj[jsons[i].name] = temp;			
